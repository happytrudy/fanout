...
func (t *Tunnel) startSingBox() error {
	t.lifecycleMu.Lock()
	defer t.lifecycleMu.Unlock()
	return t.startSingBoxLocked()
}

// startSingBoxLocked starts this tunnel while the lifecycle lock is held.
func (t *Tunnel) startSingBoxLocked() error {
	...
	t.mu.Unlock()
	if engine == nil {
		return fmt.Errorf("内嵌 sing-box 不可用")
	}
...
func (t *Tunnel) restartWithCredential(next SocksCred) error {
	t.lifecycleMu.Lock()
	defer t.lifecycleMu.Unlock()
	if t.snapshot().Status == "stopped" {
		return fmt.Errorf("隧道已停止")
